class Http {
	private HttpInstance = class {
		...
		public constructor(url, type, data, async, callback, useProperties = true) {
			// add FB user login status properties
			// if (useProperties) url = URLProperties.properties(url);
