...
var defaultNewBank = (function (_super) {
    ...
    defaultNewBank.prototype.init = function () {
        ...
        this.addText(this, 15, 524, 400, 30, 25, "left").text = { en: "Coins for level: ", es: "para el nivel: ", pt: "para o nível: " }[GlobelSettings.language] + userLevel;
        this.createDailyBar();
    };
    defaultNewBank.prototype.createDailyBar = function () {
        ...
        this.clickTip.text = { en: "Free Hourly Bonus", es: "Bonos a cada dos horas", pt: "Bônus a cada duas horas" }[GlobelSettings.language];
        if (spinBar.enabled) {
    ...
    defaultNewBank.prototype.enableCollectBonus = function () {
        ...
        var userLevel = eval("PlayerConfig.player('score').level");
        this.hourly_bonus_txt.text = "$" + Math.floor(eval("PlayerConfig.player('bonus.hourly_bonuses')")[userLevel]);
        ...
        var coin_mc = Com.addMovieClipAt(this.dailyBar, mcf, "coins", -47, -98);
        var coinPic = Com.addBitmapAt(this.dailyBar, "defaultNewBank_json.bank_coin_big", 245, -50);
        var tw = egret.Tween.get(coinPic);
        coinPic.scaleX = 1.5;
        coinPic.scaleY = 1.5;
        tw.to({ scaleX: 0.5, scaleY: 0.5, x: 335, y: 110 }, 1000, egret.Ease.circIn);
        tw.to({ visible: false }, 1);
    ...
    defaultNewBank.prototype.createBankItem = function (product, index) {
        ...
        var priceText = (currencyIsBrl ? "R$" : "$") + product["price"] * (currencyIsBrl ? 4 : 1);
        Com.addBitmapAt(item, "defaultNewBank_json.bg_patch_price", 376, 18);
        var price = Com.addTextAt(item, 376, 18, 82, 32, 20, false, false);
        price.fontFamily = "Righteous";
        price.text = priceText;
        price.size = 20 - (priceText.length - 5) * 2;
    ...
    defaultNewBank.prototype.onCoinsCollect = function (event) {
        ...
        this.onRemove(null);
        this.removeChild(this.dailyBar);
        setTimeout(this.recreateDalayBarrowMove.bind(this), 5000);
    };
    defaultNewBank.prototype.recreateDalayBarrowMove = function () {
        if (this.stage) {
            this.createDailyBar();
        }
    };
    ...
    defaultNewBank.prototype.onClose = function (event) {
        this.dispatchEvent(new egret.Event("exit_bank"));
