...
var H5_po = (function (_super) {
    __extends(H5_po, _super);
    function H5_po(configUrl) {
        return _super.call(this, configUrl) || this;
    }
    Object.defineProperty(H5_po, "classAssetName", {
        get: function () {
            return "Standard_6903_" + GlobelSettings.language; //subclass must override
        },
        enumerable: true,
        configurable: true
    });
    H5_po.prototype.init = function () {
        this.bgAssetName = "random_8733_" + GlobelSettings.language.toUpperCase() + "_png";
        _super.prototype.init.call(this);
        var closeBtnRect = new egret.Rectangle(679,130,41,36); //range of close button
        var comfirmBtnRect = new egret.Rectangle(328,611,282,84); //range of comfirm button
        var coinRect = new egret.Rectangle(220, 310, 410, 45);
        var coinColor = 0xffe25a;
        var priceRect = new egret.Rectangle(250, 475, 425, 45);
        var priceColor = 0xFFFFFF;
        var disCountRect = new egret.Rectangle(320, 370, 225, 35);
    ...
    // }
    H5_po.prototype.poOverplusOver = function () {
        this.onClose(null);
    };
    H5_po.prototype.onRemove = function () {
        this.removeChildren();
        // this.addChild( this.bg );
        // this.addChild( this.closeButton );
    };
    H5_po.prototype.buyProduct = function (event) {
        var ev = new egret.Event(GenericModal.MODAL_COMMAND);
        ev["cmd"] = "buyProduct";ev["productType"] = 1;
        this.dispatchEvent(ev);
    };
    H5_po.prototype.createAlphaButton = function (rect, fun) {
    ...
    H5_po.prototype.createText = function (rect, color, useFilters, bold) {
    ...
    H5_po.prototype.createCoinsTest = function (rect, color, txt, size, dropShaddow) {
        if (dropShaddow === void 0) { dropShaddow = false; }
        var bitmap = Com.addBitmapAt(this, "Po_icons_json.dinero", rect.x + (rect.width + txt.textWidth) * 0.5 + 10, rect.y + rect.height * 0.5);
        bitmap.anchorOffsetY = bitmap.height >> 1;
        if (!dropShaddow)
            bitmap.filters = [MatrixTool.colorMatrix(0.33, 0.33, 1)];
        return bitmap;
    };
    H5_po.prototype.createDisCountLine = function (disCount) {
...
__reflect(H5_po.prototype, "H5_po");
